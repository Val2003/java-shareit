...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto updateItem(Long itemId, ItemDto newItemDto, Long userId) {
        ...
        if (oldItem.getOwner().getId().equals(owner.getId())) {
            ...
            Item item = EntityMapper.INSTANCE.updatedItem(newItemDto, oldItem);
            item.setOwner(owner);
            itemRepository.save(item);
            log.info("Item ID {} was updated by user ID {}", itemId, userId);
            return EntityMapper.INSTANCE.toItemDto(item);
        } else {
    ...
    public AnswerItemDto getItem(Long itemId, Long userId) {
        ...
        List<CommentDto> comments = commentRepository.findAllByItem_Id(item.getId())
                .stream()
                .map(EntityMapper.INSTANCE::toCommentDto)
                .collect(Collectors.toList());

        InfoBookingDto lastBooking = EntityMapper.INSTANCE.toInfoBookingDto(bookingRepository
        ...
        InfoBookingDto nextBooking = EntityMapper.INSTANCE.toInfoBookingDto(bookingRepository
        ...
        return EntityMapper.INSTANCE.toAnswerItemDto(item, lastBooking, nextBooking, comments);
    }
    ...
    public List<AnswerItemDto> getItemsByUser(Long userId, Pageable pageable) {
        ...
        return items.stream()
                .map(item -> EntityMapper.INSTANCE.toAnswerItemDto(
                        item,
                        EntityMapper.INSTANCE.toInfoBookingDto(lastBookings.get(item.getId())),
                        EntityMapper.INSTANCE.toInfoBookingDto(nextBookings.get(item.getId())),
    ...
    public CommentDto createComment(Long itemId, Long userId, CommentDto commentDto) {
        ...
        if (bookingRepository.isItemWasUsedByUser(itemId, userId, LocalDateTime.now())) {
            ...
            return EntityMapper.INSTANCE.toCommentDto(commentRepository.save(comment));
        } else {
